import { View, Image, Text as ReactText, StyleSheet, Dimensions, VirtualizedList } from 'react-native'
...
import { AnimatedLoader, Container, Header, ModalLoader, PremiumButton, Text } from '../../components'
import { Colors, Fonts, Images } from '../../res'
import { Typography, hp, wp } from '../../global'
import { formatDate, useGlobalContext, StorageManager, ApiServices, stopConversationsListener } from '../../services'
...
const Messages = (props: any) => {
    ...
    const { conversations, coversationLoading, currentUser, updateCurrentUser, language } = useGlobalContext()

    const onItemPress = (item: any, otherUserData: any) => {
        props.navigation.navigate('SingleChat', { conversationData: item, otherUserData: otherUserData, from: 'messages' })
    }
    ...
    const renderConversations = ({ item }: any) => {
        ...
        let currentUserId = currentUser?.id === 'guardian' ? currentUser?.user?.id : currentUser?.id
        ...
        const isBlockedYou = convDetails?.participantsBlockFlag[currentUser?.id]?.blockStatus === true ?
            true : false

        let hideLatestMessage = false
        if (item?.messages) {
            hideLatestMessage = Object.keys(item?.messages).length === 0 ? true : false
        }

        return (
            <Ripple style={[Styles.itemContainer, Styles.itemHeight, { flexDirection: Rtl ? 'row-reverse' : 'row' }]}
                onPress={onItemPress.bind(null, item, otherUserData)}
            >
                <View style={Styles.profilePictureCon}>
                    {
                        otherUserData?.image && otherUserData?.image?.length !== 0 && !isBlockedYou ?
                            <>
                ...
                <View style={[Styles.itemInnerCon, Styles.itemHeight, { flexDirection: Rtl ? 'row-reverse' : 'row' }]}>
                    ...
                    <View style={Styles.timeCon}>
                        {
                            !hideLatestMessage &&
                            <ReactText style={[Styles.itemMessage, { alignSelf: Rtl ? 'flex-start' : 'flex-end' }]}>
                                {formattedDate}
                            </ReactText>
                        }
                        {
                            unReadCount && unReadCount !== 0 && !hideLatestMessage ?
                                <View style={[Styles.unReadCountCon, { alignSelf: Rtl ? 'flex-start' : 'flex-end' }]}>
                                    <ReactText numberOfLines={1}
    ...
    return (
        <Container>
            ...
            <Header
                ...
                customConponent={() => currentUser?.role === 'guardian' && (
                    <View style={[Styles.gaurdianHeader]}>
                        <OptionsMenu
                            button={Images.verticalDots}
                            buttonStyle={Styles.menuBtn}
                            destructiveIndex={2}
                            options={[t(LanguageKeys.changePassword), t(LanguageKeys.logOut), t(LanguageKeys.cancel)]}
            ...
            {currentUser?.role === 'guardian' && <CommonActions
