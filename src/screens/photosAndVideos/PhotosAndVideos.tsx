import { View, StyleSheet, Image, FlatList, ScrollView, Dimensions, ActivityIndicator, Modal, Text as ReactText } from 'react-native'
...
import { AnimatedLoader, Button, ButtonPicker, Container, Header, ImagePicker, ModalLoader, Text } from '../../components'
import { hp, Typography, wp } from '../../global'
import { Colors, Fonts } from '../../res'
import { Images } from '../../res'
import { LanguageKeys, CheckRtl } from '../../languages'
import { ApiServices, flashErrorMessage, flashSuccessMessage, isIOS } from '../../services'
...
const PhotosAndVideos = (props: any) => {
    ...
    const setUserData = async () => {
        ...
        if (media) {
            const { cover_image, youtube_url, primary_image, public_gallery, private_gallery } = media
    ...
    const onArrowUpPress = (item: any, from: any) => {
        ...
        if (from === 'public_gallery') {
            ...
            ApiServices.moveMedia(params).then((res: any) => {
    ...
    const onTryAgainPress = (item: any, index: any, from: any) => {
        ...
        forceUpdate()
        imageResizer(item).then((res: any) => {
            res.type = item?.type
            ApiServices.imageUpload(res, from, youtubeURL).then(async (res: any) => {
                if (res) {
                    const { public_gallery, private_gallery } = res
                    currentUser.media = res
                    await setData(storageKeys.USER, currentUser)
                    item.uploadSuccessful = true
                    item.uri = imagePicker.from === 'public_gallery' ?
    ...
    const RenderPhotosList = ({ item, index, from }: any) => {
        ...
        return (
            <View style={{
                ...Styles.itemOuterCon, paddingRight: Rtl ? 0 : wp(6),
                ...
                {
                    uploadSuccessful && from === 'private_gallery' && publicPhotos?.length < 10 ?
                        <View style={{ ...Styles.upBtnCon, alignItems: Rtl ? 'flex-start' : 'flex-end', }}>
                            <Ripple style={Styles.upBtn}
                ...
                {
                    uploadSuccessful && from !== 'private_gallery' && privatePhotos?.length < 10 ?
                        <View style={{ ...Styles.downBtnCon, alignItems: Rtl ? 'flex-start' : 'flex-end', }}>
                            <Ripple style={Styles.downBtn}
                ...
                {
                    uploadSuccessful &&
                    <Ripple style={{ ...Styles.menuButtonCon, ...Styles.shadowTwo, alignSelf: Rtl ? 'flex-end' : 'flex-start' }}
    ...
    const onUpoadPicture = (imageObj: {
        ...
        showUploadingLoader()
        hideImagePicker()
        ApiServices.imageUpload(imageObj, imagePicker.from, youtubeURL).then(async (res: any) => {
            if (res) {
    ...
    const AddProfilePictureBtn = () => (
        <View style={{ ...Styles.addProfilePicBtnCon, alignItems: Rtl ? 'flex-end' : 'flex-start', }}>
            {
                uploadingProfileLoader ?
                    <View style={{
                        ...Styles.addPhotoBtn,
                        marginVertical: 0, marginLeft: wp(4), marginRight: wp(4),
                    }}>
                        <AnimatedLoader
                            text="Uploading..."
                            visible={true}
                    ...
                    profileImage.length === 0 ?
                        ...
                        <View style={Styles.profileImageCon}>
                            ...
                            />
                            <View style={{ ...Styles.downBtnConProfile, top: hp(-2), alignItems: Rtl ? 'flex-start' : 'flex-end', }}>
                            ...
                            {publicPhotos?.length < 10 ? <View style={{ ...Styles.downBtnConProfile, alignItems: Rtl ? 'flex-start' : 'flex-end', }}>
                                <Ripple style={Styles.downBtn}
                                    onPress={onArrowDownPress.bind(null, { uri: profileImage }, 'primary_image')}
                                >
                                    <AntDesign
                                        name='arrowdown'
                                        color={Colors.color2}
    ...
    const AddPhotoBtn = (from: any) => {
        return (
            <Ripple style={{
                ...Styles.addPhotoBtn, marginRight: Rtl ? 0 : wp(6),
        ...
        return (
            <View style={Styles.listOuterCon}>
                ...
                <FlatList
                    horizontal
                    data={data}
                    renderItem={({ item, index }) => <RenderPhotosList item={item} index={index} from={from} />}
    ...
    const deleteCoverImage = () => {
        ...
        ApiServices.deleteImage(params).then(() => {
            setCoverImage('')
            hideLoader()
    ...
    const hideImagePicker = () => setImagePicker({
        visible: false,
        from: '',
    })
    ...
    const onImagePublicPrivateSelection = (images: any) => {
        hideImagePicker()
        if ((publicPhotos.length === 10 && imagePicker.from === 'public_gallery') ||
            ...
            if (imagePicker.from === 'public_gallery') {
                ...
                if (length > 10) {
                    flashErrorMessage(`You cannot add more than 10 images in public gallery you can add ${10 - publicPhotos.length} more images`)
                }
                else {
                ...
                if (length > 10) {
                    flashErrorMessage(`You cannot add more than 10 images in private gallery you can add ${10 - privatePhotos.length} more images`)
                }
                else {
            ...
            if (newImages.length <= 10) {
                newImages.forEach((element: any) => {
                    if (!element.uploadSuccessful) {
                        imageResizer(element).then((res: any) => {
                            res.type = element?.type
                            ApiServices.imageUpload(res, imagePicker.from, youtubeURL).then(async (res: any) => {
                                if (res) {
                                    const { public_gallery, private_gallery } = res
                                    currentUser.media = res
                                    await setData(storageKeys.USER, currentUser)
                                    element.uploadSuccessful = true
                                    element.uri = imagePicker.from === 'public_gallery' ?
                                        public_gallery[public_gallery.length - 1] :
                                        private_gallery[private_gallery.length - 1]
                                    forceUpdate()
                                }
    ...
    const onImagePrivatePublicDelete = (item: any, from: any) => {
        ...
        ApiServices.deleteImage(params).then((res) => {
            if (from === 'public_gallery') {
                _.remove(publicPhotos, function (n: any) {
                    return n.uri == uri;
                });
    ...
    const onButtonPickerButtonPress = (item: any) => {
        ...
        hideButtonPicker()
        if (value === 'delete' &&
            (buttonPickerVisible.from === 'private_gallery' || buttonPickerVisible.from === 'public_gallery')) {
            onImagePrivatePublicDelete(buttonPickerVisible.item, buttonPickerVisible.from)
        }
        else if (value === 'delete' && buttonPickerVisible.from === 'cover_image') {
            deleteCoverImage()
        }
        else if (value === 'delete' && buttonPickerVisible.from === 'primary_image') {
            deleteProfileImage()
        }
    }
    ...
    const onSavePress = () => {
        if (youtubeURL.length === 0) {
            ...
            ApiServices.imageUpload({}, {}, youtubeURL).then(async (res) => {
                currentUser.media = res
                await setData(storageKeys.USER, currentUser)
    ...
    const RenderCover = () => {
        return (
            uploadingCoverLoader ?
                ...
                :
                coverImage.length === 0 ?
                    <Ripple style={Styles.coverPhoto}
                        onPress={onAddCoverPress}
                    >
                        <Image
                            source={Images.camera}
                            resizeMode='contain'
        ...
        <Container>
                        ...
                        <Modal visible={toolTipVisible}>
                            <View style={Styles.modalWrapper}>
                                ...
                                <View style={Styles.tootltipTextWrapper}>
                                    ...
                                    <Text style={Styles.tootltipTitle}>Honoring Islamic Values</Text>
                                    <Text style={Styles.tootltipDesc}>We request you to uphold modesty, inviting blessings and mercy from Allah. Female profile pictures are blurred by default. They can decide who gets to see their images.</Text>
                                    <Text style={Styles.tootltipTitle}>Quranic Versed:</Text>
                                    <Text style={Styles.tootltipText}>"And tell the believing women to lower their gaze and guard their private parts and not expose their adornment except that which (necessarily)..." (Qur'an 24:31)</Text>
                                    <ReactText style={[Styles.tootltipTitle, { marginTop: 30 }]}>Hadith:</ReactText>
                                    <Text style={Styles.tootltipText}>"Modesty is part of faith and faith is in Paradise, but obscenity is a part of hardness of the heart and hardness of the heart is in Hell." (Sahih Muslim)</Text>
                                </View>
            ...
            <ImagePicker
                ...
                onImageSelection={(res: any) => {
                    hideImagePicker()
                    if (imagePicker.from === 'public_gallery' || imagePicker.from === 'private_gallery') {
