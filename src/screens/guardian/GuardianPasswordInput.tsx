import { View, StyleSheet, KeyboardAvoidingView, TextInput, TouchableOpacity, StatusBar, Image } from 'react-native'
import React, { useState } from 'react'
import LinearGradient from 'react-native-linear-gradient'
import { Button, Header, ModalLoader, SlideShowContainer, Text } from '../../components'
import { Colors, Fonts, Images } from '../../res'
import { Typography, hp, wp } from '../../global'
import { Animation } from '../../animations'
import { CheckRtl, LanguageKeys } from '../../languages'
import { ApiServices, StorageManager, checkEmpty, flashErrorMessage, flashSuccessMessage, getConversationsOnce, useGlobalContext } from '../../services'
...
const GuardianPasswordInput = ({ navigation, route }: any) => {
    ...
    const { updateCurrentUser, updateConversations, updateConversationLoading } = useGlobalContext()
    const Rtl = CheckRtl()
        ...
        getConversationsOnce(currentUser?.user?.id, async (snapshot: any) => {
            if (snapshot) {
                let conversationsData: any = snapshot.val() ?
                    ...
                    : []
                await setData(storageKeys.CONVERSATIONS, conversationsData).then(async () => {
                    try {
                ...
                >
                    <Animation style={Styles.contentContainer}>
                        <Text style={Styles.heading}>
                            {LanguageKeys.enterYourPassword}
                        </Text>
                        <View style={Styles.passwordInputCon}>
                            <TextInput
                                style={[Styles.passwordInput, { textAlign: Rtl ? 'right' : 'left' }]}
                                value={password}
