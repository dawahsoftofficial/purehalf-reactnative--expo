import { View, StyleSheet, Text as ReactText, KeyboardAvoidingView, TouchableOpacity, ScrollView } from 'react-native'
...
import { ApiServices, StorageManager, flashSuccessMessage, isIOS, useGlobalContext } from '../../services';
...
const VerifyWaliCode = ({ navigation }: any) => {
    ...
    const calculateRemainingTime = async () => {
        const startTimeString: any = await getData(storageKeys.VERIFICATION_CODE_TIMER)
        if (startTimeString) {
    ...
    const onResendCode = () => {
        ...
        ApiServices.resendWaliVerificationCode()
            .then(async () => {
                ...
                await setData(storageKeys.VERIFICATION_CODE_TIMER, startTime.toString())
                calculateRemainingTime()
            ...
            >
                ...
                >
                    <View style={Styles.descriptionCon}>
                        ...
                        <ReactText style={[Styles.description, { fontFamily: Fonts.APPFONT_SB, color: Colors.color1 }]}>
                            {guardian?.email}
                        </ReactText>
                        <ReactText style={Styles.description}>
