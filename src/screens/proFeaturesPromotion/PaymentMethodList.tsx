import { View, Modal, StyleSheet, StatusBar, TouchableOpacity, Platform, FlatList, Image, Linking } from 'react-native'
...
const paymentMethodList = (props: any) => {
    ...
    const getData = () => {
        ApiServices.getPaymentInfo()
            .then((res: any) => {
                setPaymentMethods(res?.sort((a: any, b: any) => a?.position - b?.position))
            })
            .catch(() => { });
    }
    ...
    const onItemPress = (item: any) => {
        let msg = `Salaam, its ${currentUser?.first_name + " " + currentUser?.last_name}, I would like to purchase a ${selectedPackage?.packageType} Subscription (${selectedPackage?.product?.priceString}). My Payment number is ${JSON.stringify(currentUser?.id * 3146)}. Jazak Allah Khayran`;
        let phoneWithCountryCode = "923330093158";
        ...
        return (
            ...
            >
                ...
                <Text style={Styles.itemLabel}>
                    {item?.detail?.SERVICE || item?.detail?.ACCOUNT_NAME || item?.detail?.BANK}
                </Text>
            </Ripple>
        )
    }
    ...
    return (
        ...
        >
            <TouchableOpacity style={Styles.container}
                ...
                onPress={onClose}
            >
                <StatusBar backgroundColor={Colors.blackRGBA70} barStyle={'light-content'} />
                <Animation
                    duration={500}
                >
