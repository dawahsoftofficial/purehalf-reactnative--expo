...
const DiscountPackagesList = (props: any) => {
    ...
    const [selectedPackage, setSelectedPackage] = useState<any>({ index: 2, data: data[2] })
    ...
    const renderPackages = ({ item, index }: any) => {
        const currencySymbol = item?.product?.priceString?.match(/^[^\d]+/)[0]?.trim();
            ...
            >
                <View style={{
                    ...
                    <Text style={Styles.number}>
                        {
                            item?.packageType === 'WEEKLY' ? '1' :
                                item?.packageType === 'MONTHLY' ? '1' :
                                    item?.packageType === 'TWO_MONTH' ? '2' :
                                        item?.packageType === 'THREE_MONTH' ? '3' :
                                            item?.packageType === 'SIX_MONTH' ? '6' :
                                                item?.packageType === 'ANNUAL' ? '12' : ''
                        }
                    </Text>
                    <View style={{ paddingLeft: Rtl ? 0 : wp(2), paddingRight: Rtl ? wp(2) : 0, marginTop: hp(-0.3) }}>
                ...
                </View>
                {
                    ...
                    <View style={Styles.itemSecondCon}>
                        <Text style={{ ...Styles.description, alignSelf: Rtl ? 'flex-start' : 'flex-end', marginLeft: Rtl ? wp(1) : 0 }}>
                            bestValue
                        </Text>
                        <View style={{ ...Styles.saveCon, alignSelf: Rtl ? 'flex-start' : 'flex-end' }}>
    ...
    return (
        <View style={{ flex: 1 }}>
            <Ripple style={{
                ...Styles.itemContainer,
                flexDirection: Rtl ? 'row-reverse' : 'row',
                borderColor: selectedPackage?.data?.title === 'free' ? Colors.color38 : Colors.color2,
                backgroundColor: selectedPackage?.data?.title === 'free' ? Colors.color39 : 'transparent',
            }}
            ...
            >
                <View style={{
                    ...
                    <View style={{ width: "100%", paddingLeft: Rtl ? 0 : wp(2), paddingRight: Rtl ? wp(2) : 0, marginTop: hp(-0.3) }}>
                        <Text style={Styles.description}>
