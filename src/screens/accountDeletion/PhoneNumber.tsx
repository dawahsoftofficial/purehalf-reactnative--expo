...
import { checkEmpty, Firebase, flashErrorMessage, useGlobalContext } from '../../services'
...
const PhoneNumber = () => {
    ...
    const [phoneNumber, setPhoneNumber] = useState(currentUser?.phone_number?.slice(3, currentUser?.phone_number?.length))
    ...
    const onContinuePress = () => {
        if (selectedCountry.dial_code === '+92' && (phoneNumber.length < 10 || phoneNumber.length > 12)) {
            ...
            if (phoneNumberWithCode !== currentUser?.phone_number) {
                flashErrorMessage('Please provide correct phone number ( The number with which you created account )')
            }
            else {
                setLoading(true)
                Firebase.sendVerificationCode(phoneNumberWithCode).then((verificationRes: any) => {
                    setLoading(false)
