import { View, FlatList, StyleSheet, Dimensions, Image, ActivityIndicator, TouchableOpacity, Text as ReactText } from 'react-native'
...
const RequestedList = (props: any) => {
    ...
    const RenderItemContent = ({ item }: any) => {
        return (
            <View style={Styles.itemInnerCon}>
                <ReactText style={{ ...Styles.itemName, alignSelf: Rtl ? 'flex-end' : 'flex-start' }}
                    numberOfLines={2}
                >
                    {item?.full_name}
                </ReactText>
                {(item?.city || item?.country) &&
                    <ReactText
                        style={{ ...Styles.itemLocation, alignSelf: Rtl ? 'flex-end' : 'flex-start' }} numberOfLines={1}>
    ...
    const RenderButtons = ({ item }: any) => (
        from === 'othersRequests' ?
            <View style={{ ...Styles.buttonsOuterCon, flexDirection: Rtl ? 'row-reverse' : 'row' }}>
                <RenderSignleButton
                    name="close"
            ...
            :
            <View style={{ ...Styles.buttonsOuterCon, flexDirection: Rtl ? 'row-reverse' : 'row', justifyContent: 'flex-end' }}>
                <RenderSignleButton
                    name="delete"
    ...
    const renderList = ({ item }: any) => {
        return (
            ...
            >
                ...
                <View style={{ ...Styles.itemContentCon, flexDirection: Rtl ? 'row-reverse' : 'row' }}>
                    <RenderItemContent
                        item={item}
                    />
                    <RenderButtons
    ...
    const renderEmptyList = () => (
        <View style={Styles.emptyListContainer}>
            ...
            <Text style={Styles.emptyListText}>
                {from === "othersRequests" ? LanguageKeys.noPhotoRequests : LanguageKeys.noPhotoRequested}
            </Text>
        </View>
    )
    ...
    const onButtonPickerButtonPress = (item: any) => {
        ...
        if (value === 'delete') {
            ...
            ApiServices.privatePhotoRemoveRequest(userId).then(() => {
            ...
            const userId = buttonPickerVisible.userId
            ApiServices.privatePhotoAcceptRequest(userId).then(() => {
                _.remove(data, function (n: any) {
                    return n.id === userId
                });
                forceUpdate()
            ...
            const userId = buttonPickerVisible.userId
            ApiServices.privatePhotoRejectRequest(userId).then(() => {
                _.remove(data, function (n: any) {
                    return n.id === userId
                });
                forceUpdate()
