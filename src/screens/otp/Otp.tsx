...
import { View, Text as ReactText, TouchableOpacity, KeyboardAvoidingView, Keyboard } from 'react-native'
...
import { ApiServices, Firebase, StorageManager, flashErrorMessage, flashSuccessMessage, isIOS, setRevenueCat, useGlobalContext } from '../../services'
...
const Otp = (props: any) => {
    ...
    const imagesContainerDisabled = props?.from && props.from === 'AccountDeletion' ? true : false
    const textColor = props?.from && props.from === 'AccountDeletion' ? Colors.color1 : Colors.color2


    let phoneNumber = props?.route?.params?.phoneNumber
    const [phoneNumberFirebaseRes, setPhoneNumberFirebaseRes] = useState(props?.route?.params?.phoneNumberFirebaseRes)
    const [isKeyboardOpen, setIsKeyboardOpen] = useState(false);
    const { currentUser, updateCurrentUser } = useGlobalContext()
    const [continueLoader, setContinueLoader] = useState(false)
    const [continueLoaderMessage, setContinueLoaderMessage] = useState('Verifying...')

    const CELL_COUNT = 6;
    const [value, setValue] = useState('');
    const ref = useBlurOnFulfill({ value, cellCount: CELL_COUNT });
    const [propsCell, getCellOnLayoutHandler] = useClearByFocusCell({ value, setValue });
    ...
    const onLoggedIn = async (userObj: any) => {
        ...
        if (userObj?.results?.first_name) {
            if (
                !userObj?.results?.media || !userObj?.results?.media?.primary_image ||
                userObj?.results?.media?.primary_image?.length === 0
            ) {
                navigateTo('ProfilePicture')
            }
            else if (userObj?.results?.membership_status === null || userObj?.results?.membership_status === 0) {
                props.navigation.reset({
                    index: 0,
                    routes: [{
    ...
    const onOTPVerified = async () => {
        if (!currentUser) {
            ApiServices.loginUser(phoneNumber, (currentUser) => onLoggedIn(currentUser))
        } else {
            ...
            if (currentUser?.first_name) {
                if (
                    !currentUser?.media || !currentUser?.media?.primary_image ||
                    currentUser?.media?.primary_image?.length === 0
                ) {
                    navigateTo('ProfilePicture')
                }
                else if (currentUser?.membership_status === null || currentUser?.membership_status === 0) {
                    props.navigation.reset({
                        index: 0,
                        routes: [{
                            name: 'ProFeaturesPromotion',
        ...
        if (from === 'AccountDeletion') {
            if (!purposeOfLeaving) {
                ...
                if (props?.type === "email") {
                    ...
                    Firebase.matchOTP(phoneNumberFirebaseRes, code).then(() => {
            ...
            setContinueLoaderMessage('Verifying...')
            setContinueLoader(true)
            Firebase.matchLoginVerificationCode(phoneNumberFirebaseRes, code).then(() => {
                // setContinueLoader(false)
            })
                .catch(hideContinueLoader)
        }
    ...
    return (
        <SlideShowContainer
            disabled={imagesContainerDisabled}
        >
            ...
            >
                <View style={[Styles.container, { height: imagesContainerDisabled ? 'auto' : hp(100) }]}>
                    <Animation>
                        <View style={Styles.headingCon}>
                            <Text style={[Styles.description, { color: textColor }]}>
                                {props?.type === "email" ? LanguageKeys.emailSent : LanguageKeys.smsSent}
                            </Text>
                        </View>
                        <View style={Styles.headingCon}>
                        ...
                        <View style={Styles.otpInputCon}>
                            <CodeField
                                ...
                                renderCell={({ index, symbol, isFocused }) => (
                                    <ReactText
                                        key={index}
                                        style={[Styles.cell, isFocused && Styles.focusCell, { borderColor: textColor, color: textColor }]}
                        ...
                        {
                            seconds > 0 &&
                            <View style={{ ...Styles.tryAgainCon, alignSelf: Rtl ? 'flex-end' : 'flex-start', flexDirection: Rtl ? 'row' : 'row-reverse' }}>
                        ...
                        <View style={{ ...Styles.continueBtnCon, marginBottom: !isIOS && isKeyboardOpen ? hp(-11) : hp(3) }}>
                            <Button
