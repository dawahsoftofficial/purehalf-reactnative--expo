...
import { ApiServices, Firebase, flashErrorMessage, isIOS, useGlobalContext } from '../services';
    ...
    useFocusEffect(
        React.useCallback(() => {
            if (data?.detail?.personality_id_value?.length) {
                setMatchingData(data?.detail?.personality_id_value?.slice(0, 4)
                    ?.filter((userData: any) => currentUser?.detail?.personality_id?.includes(userData?.id)))
                ...
                ApiServices.getCurrentUserDetail()
                    .then((res: any) => {
                        ...
                        if (
                            ...
                            navigation.navigate('ProFeaturesPromotion', { navigateTo: 'goBack' })
                        } else {
                            resolve('premiumUser');
                        }
                    })
    ...
    const image = parallax ? (
        ...
        <View style={{ ...Styles.profileImageCon, ...Styles.shadow, left: 0, right: 0, bottom: 0 }}>
            {!profileImageError && data?.media?.primary_image ?
                <Image
                    source={data?.media?.primary_image ? { uri: data?.media?.primary_image } : Images.userPlaceholderVertical}
        ...
        return (
            <View style={Styles.nullSlideInnerContainer}>
                <View style={Styles.nullUserInfoContainer}>
                    ...
                    <Text style={Styles.userNullTxtName}>{t('noRcommendedUserAvailable')}</Text>
                </View>
    ...
    return (
        <View style={Styles.slideInnerContainer}>
            ...
            <View style={Styles.userDataContainer}>
                <View>
                    <Text style={Styles.userInfoTxtName}>{chatUserData?.name}, {chatUserData?.age}</Text>
                    {chatUserData?.city && <Text style={Styles.userInfoTxt}>{chatUserData?.city}{chatUserData?.city && chatUserData?.country && ", "}{chatUserData?.country}</Text>}
                </View>
                {matchingData?.length ? <View
                    style={{
                ...
                <View style={Styles.textContainer}>
                    <Animatable.View ref={likeIconRef} style={[Styles.bottomBtnContainer, { backgroundColor: Colors.color22 }]}>
                        <Ripple
                    ...
                    <Animatable.View ref={unLikeIconRef} style={[Styles.bottomBtnContainer, { backgroundColor: Colors.color22 }]}>
                        <Ripple
                            style={Styles.btnWrapper}
                    ...
                    <Ripple
                        ...
                        <Text style={[Styles.btnTxt, { color: Colors.color2 }]}>Message</Text>
                    </Ripple>
                </View>
            </View>
            <LinearGradient colors={['rgba(0,0,0,0)', 'rgba(0,0,0,0)', 'rgba(0,0,0,.7)', 'rgba(0,0,0,1)']} locations={[0, 0.3, 0.8, 1]}
                style={{
                    position: 'absolute',
                    left: 0,
                    right: 0,
