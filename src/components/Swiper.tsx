...
const _renderItem = ({ item, index, swipeNext, onLikePress, onPassPress, onPress }: any) => {
    return (
        <SliderEntry
            data={item}
            even={(index + 1) % 2 === 0}
...
const SwiperComponent = ({ onPress }: { onPress: (value?: boolean) => void }) => {
    ...
    const onPassPress = (id: number) => {
        ...
        ApiServices.topPicks(params).then((res) => {
            swiper?.current?.snapToNext();
        }).catch();
    };
                ...
                {users?.length ? (
                    <Carousel
                        ...
                        renderItem={({ item, index }) =>
                            _renderItem({ item, index, swipeNext, onLikePress, onPassPress, onPress })
                        }
